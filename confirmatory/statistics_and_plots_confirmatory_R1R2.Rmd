---
title: "Confirmatory: There is a power law of joint communicative effort and it reflects communicative work (version R2)"
output:
  html_document:
date: "2025-08-08"
---
...
# features
features <- read.csv(paste0(curfolder,"/features_exploratory/features_processed.csv"))
...
# slopes exploratory extension features
```{r}

# Notes
#+ avg_dist_exploratory: change, avg(A(pre,post), B(pre,post))
#+ prepost_convergence: convergence, pre(A,B)-post(A,B)
features <- read.csv(paste0(curfolder,"/confirmatory/features_exploratory/features_processed.csv"))

features$Session <- ifelse(features$Session==1, "pre", "post")

# Function to split number and remove leading zeros
split_number <- function(num) {
  left <- as.integer(substr(as.character(num), 1, nchar(num)/2))
  right <- as.integer(substr(as.character(num), (nchar(num)/2 + 1), nchar(num)))
  return(c(left, right))
}

# Apply the function to each number
result <- t(sapply(unique(all$pair), split_number))
# Create a data frame
result <- as.data.frame(result)
colnames(result) <- c("A", "B")
result$pair <- unique(all$pair)

# only keep the participants in this sample
features <- subset(features, features$ParticipantNr%in%c(result$A, result$B))

# now go through 
all$change_features <- NA
all$change_convergence <- NA


# Pivot the data to have features as columns and scores as values
feats <- cbind.data.frame(features$Feature, features$Score)
colnames(feats) <- c("Feature", "Score")
pivot_features <- feats  %>%
  group_by(Feature) %>%
  mutate(row = row_number()) %>%
  pivot_wider(names_from = Feature, values_from = Score) %>%
  select(-row)

var_cov_matrix <- cov(pivot_features, use = "complete.obs")


# compute variance covariance matrix for all the features


for(i in unique(all$pair))
{
  for(f in unique(all$target))
  {
  # get the pp number
  A <- result$A[result$pair==i]
  B <- result$B[result$pair==i]
  # now select A's and B's features
  featuresApre <- features[features$ParticipantNr==A & features$Fribble==f & features$Session=="pre",]
  featuresApost <- features[features$ParticipantNr==A & features$Fribble==f & features$Session=="post",]
  featuresBpre <- features[features$ParticipantNr==B & features$Fribble==f & features$Session=="pre",]
  featuresBpost <- features[features$ParticipantNr==B & features$Fribble==f & features$Session=="post",]
  
  # change features avg(A(pre,post), B(pre,post))
  all$change_features[all$pair==i & all$target==f] <-  (mahalanobis(featuresApre$Score, featuresApost$Score, cov = var_cov_matrix)+mahalanobis(featuresBpre$Score, featuresBpost$Score, cov = var_cov_matrix))/2
  all$convergence_features[all$pair==i & all$target==f] <- mahalanobis(featuresApre$Score, featuresBpre$Score, cov = var_cov_matrix)-mahalanobis(featuresApost$Score, featuresBpost$Score, cov = var_cov_matrix)
  
  #all$change_features[all$pair==i & all$target==f] <- ((sqrt(sum((featuresApre$Score-featuresApost$Score)^2))) + sqrt(sum((featuresBpre$Score-featuresBpost$Score)^2)))/2
  # convergence pre(A,B)-post(A,B)
  #all$convergence_features[all$pair==i & all$target==f] <-  ((sqrt(sum((featuresApre$Score-featuresBpre$Scor)^2))) -sqrt(sum((featuresApost$Score-featuresBpost$Score)^2)))
  
  }
}

p1 <- ggplot(all,aes(x=avg_slopes,y=change_features, color = as.factor(target))) + geom_point(aes(group=pair))+geom_smooth(method='lm', alpha = 0, size = 0.7)+geom_smooth(method='lm', alpha=0.5, color = 'black')+ylab("change_features")+ggtitle("Exploratory: Average slope and change features")+theme_bw()+ scale_color_manual(values = my_custom_palette)
p1

p2 <- ggplot(all,aes(x=avg_slopes,y=convergence_features, color = as.factor(target))) + geom_point(aes(group=pair))+geom_smooth(method='lm', alpha = 0, size = 0.7)+geom_smooth(method='lm', alpha=0.5, color = 'black')+ylab("convergence_features")+ggtitle("Exploratory: Average slope and convergence")+theme_bw()+ scale_color_manual(values = my_custom_palette)
p2

model1 <- lmer(change_features~(1|target)+(1|pair)+avg_slopes,data=all) 
summary(model1)

model2 <- lmer(convergence_features~(1|target)+(1|pair)+avg_slopes,data=all) 
summary(model2)
...
Note that a constant proportional reduction rate yields a curvilinear reduction, but it is logarithmic reduction and not a power function.
